    ...
    layui.use(['form', 'table'], function() {
        ...
        table.render({
            ...
            cols: [[
                ...
                ,{field: 'typeName', title: '车辆类型名称', width:130, align: 'center'}
        ...
        table.on('tool(type)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){ //查看
                ...
                layer.open({
                    ...
                    area: ['900px', '300px'],
                    offset: '100px',
                    content: $('#typeview') //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
                ...
                layer.confirm('真的删除吗', function(index){
                    // 向服务端发送删除指令
                    $.get('/vehicle_type/del?id='+data.id,function (res) {
                        if(0 == res.code) {
                            layer.msg('删除成功');
                ...
                saveIndex = layer.open({
                    ...
                    area: ['900px', '300px'],
                    offset: '100px',
                    content: $('#typefrom') //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
                    ,cancel: function(index, layero){
                        layer.close(index);
                        $('#typefrom')[0].reset();
                    },
        ...
        form.on('submit(savebtn)', function (data) {
            ...
            $.ajax({
                ...
                url: '/vehicle_type/save_or_update',
