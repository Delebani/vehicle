$(function () {
    ...
    layui.use(['form', 'table'], function() {
        ...
        //工具条事件
        table.on('tool(address)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            var data = obj.data; //获得当前行数据
            var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
            var tr = obj.tr; //获得当前行 tr 的 DOM 对象（如果有的话）

            if(layEvent === 'detail'){ //查看
                form.val("addressview", {
                    "id":data.id
                ...
                layer.open({
                    ...
                    area: ['900px', '300px'],
                    offset: '100px',
                    content: $('#addressview') //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
                ...
                saveIndex = layer.open({
                    ...
                    area: ['900px', '300px'],
                    offset: '100px',
                    content: $('#addressfrom') //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
                    ,cancel: function(index, layero){
                        layer.close(index);
                        $('#addressfrom')[0].reset();
                    },
        ...
        form.on('submit(savebtn)', function (data) {
            ...
            $.ajax({
                ...
                url: '/user_address/save_or_update',
